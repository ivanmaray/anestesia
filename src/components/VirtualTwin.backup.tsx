...
interface PatientData {
  ...
  allergies: string[]
  labs: { test: string; value: number; unit: string; normal: [number, number] }[]
  pgx: { gene: string; genotype: string; risk?: string }[]
}
...
interface SimulationState {
  ...
  airwayPatency: 'patent' | 'obstructed' | 'intubated'
  ventilation: 'spontaneous' | 'assisted' | 'controlled' | 'apneic'
  hemodynamics: 'stable' | 'hypotensive' | 'hypertensive' | 'shock' | 'arrest'
  warnings: { severity: 'critical' | 'high' | 'medium' | 'low'; message: string }[]
  organEffects: {
    brain: number // 0-100
    heart: number
    lungs: number
...
const VirtualTwin: React.FC = () => {
  const [patient, setPatient] = useState<PatientData>({
    ...
    allergies: [],
    labs: [
      { test: 'Hb', value: 13.5, unit: 'g/dL', normal: [12, 16] },
      { test: 'Creatinina', value: 0.9, unit: 'mg/dL', normal: [0.6, 1.2] },
      { test: 'K+', value: 4.1, unit: 'mEq/L', normal: [3.5, 5.0] },
      { test: 'Glucosa', value: 105, unit: 'mg/dL', normal: [70, 110] },
    ],
    pgx: [
      { gene: 'CYP2D6', genotype: 'EM', risk: 'normal' },
      { gene: 'CYP2C19', genotype: 'EM', risk: 'normal' },
      { gene: 'RYR1', genotype: 'NN', risk: 'low' },
    ]
  })
